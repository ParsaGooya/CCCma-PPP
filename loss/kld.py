...
from models.normalized_flows import NormalizedFlowModel    
...
class BetaAnnealing:
    ...
    def build(self, num_batches):
        ...
        if self.num_epochs_to_hold == 0:
            ...
            self.range_epochs =   (self.num_epoch_to_warmup + self.num_epochs_to_hold) * self.num_batches 
    ...
    def __call__(self, step: int):
        ...
        if self.num_epochs_to_hold == 0:
            ...
            return self.beta_min + (self.beta - self.beta_min) * min((step /self.range_epochs), 1)
        else:
      
            cycle_pos = step % self.range_epochs
            return  self.beta_min + (self.beta - self.beta_min) * min ((cycle_pos / (self.num_epoch_to_warmup * self.num_batches)),1)
        

class KLD(lossABC):
    def __init__(self, reduction : str = 'mean', prior_flow : NormalizedFlowModel = None):
        super().__init__()
        self.reduction = reduction
        self.prior_flow = prior_flow
        if self.prior_flow is not None:
            assert self.reduction.lower() == 'sum', 'reduction must be sum for prior flow to work.'
    ...
    def forward(self, 
        ...
        if self.prior_flow is None:
            ...
            posterior_samples = self.sample(mu,  torch.sqrt(var), sample_size = self.prior_flow.flow_sample_size)
            ...
            if cond_mu is None:
                ...
                condition = cond_mu.unsqueeze(0).expand(posterior_samples.shape[0], *cond_mu.shape)
                condition = torch.flatten(condition, start_dim= 0 , end_dim=1 )
            
            # e_samples, log_det = 
            flow_output = self.prior_flow(torch.flatten(posterior_samples, start_dim= 0 , end_dim=1 ), condition =  condition)

            e_samples = torch.unflatten(flow_output.e_samples, dim = 0 , sizes = posterior_samples.shape[:2]).to(**opts)
            prior_logprob = base_dist.log_prob(e_samples).mean(0).sum(-1).to(**opts)

            log_det = torch.unflatten(flow_output.log_det, dim = 0 , sizes = posterior_samples.shape[:2]).mean(0).to(**opts)
