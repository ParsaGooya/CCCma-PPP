...
class XArrayDatasetConfig(XarrayDatasetConfigABC):
    """
    Configuration for constructing XArrayDataset instances.
    """
    ...
    def __post_init__(self):
        """
        Validate configuration and initialize internal attributes.
    ...
    def get_common_time(self):
        """
        Compute common years between model and observation datasets.

        Returns
        -------
        ndarray
            Array of common years.
        """
    ...
    def available_train_time(self):
        """
        Compute available years for training.

        Returns
        -------
        ndarray
            Array of valid training years.
        """
    ...
    def _fit_preprocessors(
        self, train_years, save=False, save_path=None, save_name=None
    ):
        """
        ...
        train_years : array-like
            Years used for fitting.
        save : bool, optional
            Whether to save fitted preprocessors.
        save_path : Path or str, optional
            Directory for saving preprocessors.
        save_name : str, optional
            Name for saved preprocessors.
    ...
    def _load_fitted_preprocessors(self, load_dir=None, load_name=None):
        """
        ...
        load_dir : Path or str, optional
            Directory containing preprocessors.
        load_name : str, optional
            Name of saved preprocessors.
        ...
        if load_dir is None:
            load_dir = (
                Path(os.environ["GLOBAL_EXP_DIR"])
    ...
    def _add_fitted_preprocessor(self, preprocessor: PreprocessModuleABC, index=0):
        """
        Add a fitted preprocessor to all pipelines.
    ...
    def build(self, years, mask=None, return_metadata=False):
        """
        Build an XArrayDataset instance.
    ...
    def _available_condiiton_methods(cls):
        """
        List supported conditioning methods.

        Returns
        -------
        list of str
            Available conditioning methods.
        """
...
class XArrayDataset(Dataset, XarrayDatasetABC):
    """
    Dataset wrapper for xarray-based data with preprocessing support.
    """
    ...
    def __post_init__(self):
        """
        Initialize dataset and load underlying data sources.
        ...
        ------
        AssertionError
            If preprocessors are not fitted or years are invalid.
        """
        ...
        self.model_dataset = _load_xarray_data(
        ...
        if self.config.condition is not None:
            self.condition_dataset = _load_xarray_data(
                self.config.condition.list_paths,
                names=self.config.condition.names,
                ensemble_mean=self.config.condition.ensemble_mean,
                selection={"ensembles": self.config.condition.info.coords["ensembles"]}
                if self.config.condition.info.coords["ensembles"] is not None
                else None,
    ...
    def get_model_indexes(self):
        """
        Compute index mapping for model dataset.

        Returns
        -------
        dict
            Mapping of dimension names to indices.
        """
    ...
    def get_obs_indexes(self, model_indexes: dict):
        """
        Compute observation indices from model indices.
    ...
    def get_cond_indexes(self, model_indexes: dict):
        """
        Compute conditioning indices from model indices.
    ...
    def added_features_dim(self):
        """
        Get number of additional features.
    ...
    def __getitem__(self, ind):
        """
        Retrieve dataset sample by index.
        ...
        -------
        dict or tuple
            Sample data dictionary and optional metadata.
