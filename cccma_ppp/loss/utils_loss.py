...
class WeightedMSE(lossABC):
    ...
    def __init__(self,
        ...
        weights_mask = torch.from_numpy(weights.where(np.isnan(weights), 1).fillna(0).to_numpy()).float()
        ...
        if self.low_ress_kernel_size is not None:
            ...
            if self.num_output_dimensions == 1:
                ...
                raise NotImplementedError('not designed for dataset with number of output dimensions > 2 (except channels)')
    ...
    def forward(self,
        ...
        assert data.shape == target.shape, f'for MSE data and target must have the same shape, got {data.shape} vs {target.shape}'
    ...
    def _aggregate(self, loss, mask):
        ...
        if self.reduction == 'mean':
            ...
            loss = torch.sum(loss, dim = tuple(-i for i in np.arange(1, self.num_output_dimensions + 1 + 1))).mean()  ## sum over output dimensions + channels

        else:
            raise NotImplementedError('Other reduction methods than sum and mean are not defined.')
...
class WeightedCRPS(lossABC):
    ...
    def forward(self,
        ...
        assert generator, 'generator cannot be False as a step_argument in Losspipeline.forward()'
        _check_generator_structure(data, target)
        ...
        if generative_modeling:  ##generative_modeling models generate a latent ensemble  (Z x batch x channels x ...)
            ...
            y_hat = torch.flatten(data, start_dim = 1, end_dim = 2) #for CRPS training an extra dimension of output ensembles should be on the left.
        else:
            y = target
            y_hat = data
        ...
        if num_samples == 1:
            ...
            crps = (y_hat - y).abs().mean(0) - (diff * weight_crps).sum(0) / num_samples**2
...
class Frobenius_norm(lossABC):
    ...
    def forward(self,
        ...
        if self.covariance_dim.lower() == 'spatial':
            covariance_truth = torch.stack([torch.cov(y[:,channel, ...].T) for channel in range(y.shape[1])])
            covariance_prediction = torch.stack([torch.cov(y_hat[:,channel, ...].T) for channel in range(y_hat.shape[1])])
            output_size = self.output_size
        elif self.covariance_dim.lower() == 'channel':
            covariance_truth = torch.cov(y.permute(1, 0, 2).reshape(y.shape[1], -1))
            covariance_prediction = torch.cov(y_hat.permute(1, 0, 2).reshape(y_hat.shape[1], -1))
            output_size = y.shape[1]

        frobenius_norm =  torch.linalg.matrix_norm(covariance_prediction - covariance_truth, ord="fro")
