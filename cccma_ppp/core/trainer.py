...
from src.cccma_ppp.core.optimization import *
from src.cccma_ppp.core.cVAE_module import cVAE
from src.cccma_ppp.data.dataloader import Dataloader
from src.cccma_ppp.generic.aggregator import MetricsAggregator
from src.cccma_ppp.loss.kld import BetaAnnealing


@dataclasses.dataclass
class TrainerConfig:
    """
    Configuration for building a training loop.
    """
    ...
    def build(
        ...
        ------
        AssertionError
            If module or configuration is invalid.
        """
...
class Trainer:
    """
    Trainer class for handling training, validation, and checkpointing.
    """
    ...
    def setup_distributed(
        self, distributed, logger, log_every_n_epochs=1, save_checkpoint=True
    ):
        """
        ...
        if resuming:
            self.log_root(
                logging.INFO,
                f"Resuming training from {self.checkpoint_dir / '*latest*.pt'}",
            )
    ...
    def train(self):
        ...
        ------
        AssertionError
            If trainer setup is incomplete.
        """
    ...
    def _train_on_batch(self, batch):
        """
        Perform training step on a batch.
    ...
    def _validate_on_epoch(self):
        ...
        ------
        RuntimeError
            If validation loader is not defined.
        """
    ...
    def _is_improved(self, validation_loss):
        """
        Determine if validation loss has improved.
    ...
    def _save_checkpoint(self, name, train_logs, validation_logs=None):
        """
    ...
    def _load_checkpoint(self, path=None, strict=True):
        """
    ...
    def raw_module(self):
        """
        Get underlying module (unwrap DDP if needed).
    ...
    def log_root(self, level, msg, *args):
        """
        Log message on root process only.
