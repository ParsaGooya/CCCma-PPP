...
def test_sampling_times_selectors():
    ...
    assert dataset._sampling_times_selectors == {
        "year": [2000, 2001],
        "lead_time": [1, 2],
    }
...
def test_dataset_length():
    dataset = bare_dataset(
        sample_coords={
            "year": np.asarray([2000, 2001, 2002, 2003]),
            "lead_time": np.asarray([1, 1, 2, 2]),
        }
    )
...
def test_input_lead_months_uses_required_lead_dimension(
    ...
    effective_input.info.coords = {
        "forecast_year": Coordinate([2000, 2001]),
        "forecast_month": Coordinate([1, 2, 3]),
    }
