...
from data.datasets import XArrayDataset, XArrayDatasetConfig
from data.utils_data import _create_train_mask, WeightsConfig
...
class TrainDataloaderConfig:
    ...
    def __post_init__(self):
        ...
        if self.train_years is None:
            ...
            if self.num_validation_years > 0:
                self.validation_years = self.dataset_config.available_train_time[- self.num_validation_years:]
        else:
            self.train_years = np.arange(self.train_years[0], self.train_years[1] + 1)

            assert set(self.train_years).issubset(set(self.available_train_years)), f'the requested train years are not available: available years: [{self.available_train_years.min()},{self.available_train_years.max()}]'

            if self.num_validation_years > 0:
                self.validation_years = np.arange(self.train_years[-1] +1 , self.train_years[-1] + 1 + self.num_validation_years)


    def setup_distributed(self, distributed : Distributed, save_path : Path | str | None = None):
        ...
        if distributed.is_root():
            self.dataset_config._fit_preprocessors(self.train_years, save = True, save_path = save_path)

        distributed.barrier()

        if distributed.distributed:
            self.dataset_config._load_fitted_preprocessors(load_dir = save_path)   #all ranks load from disk after the barrier, including rank 0, to guarantee everyone uses the exact saved state
    ...
    def build_train_loader(self, return_spatial_mask = False, reduce_spatial_mask = False):
        if not self._setup:
            raise RuntimeError('Dataloader has to be setup for distributed training first by calling .setup_distributed()')
        
        train_mask = _create_train_mask(years = self.train_years, 
    ...
    def build_validation_loader(self, return_spatial_mask = False, reduce_spatial_mask = False):
        ...
        if self.num_validation_years > 0:
            validation_mask = _create_train_mask(years = self.validation_years, 
                                            lead_times= np.arange(1, self.dataset_config.num_lead_months + 1) )
            validation_dataset = self.dataset_config.build(years = self.validation_years , mask = validation_mask, return_metadata = False)
...
class Dataloader:
    ...
    def __post_init__(self):
        ...
        self._torch_loader = DataLoader(self.dataset, 
...
def collate_batch(batch, return_spatial_mask : bool = False, reduce_spatial_mask : bool = False):
